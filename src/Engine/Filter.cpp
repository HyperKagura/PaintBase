...
Filter::Filter(QString str)
{
	QStringList temp = str.split('\'');
	QStringList rows=temp[1].split(';');
	int width = rows.size();
	kernel = QVector<QVector<qreal>>(width, QVector <qreal>(width, 0.0));
	for (int x = 0; x < width; x++)
	{
		QStringList elements = rows[x].split(',');
		for (int y = 0; y < width; y++)
		{
			kernel[x][y] = elements[y].toDouble();
		}
	}
}
